...
def root():
    """Serve the React dashboard if built, else the fallback landing page."""
    if STATIC_AVAILABLE:
...
@app.post("/reset")
def reset_all():
    """OpenEnv global reset — required by the hackathon checker."""
...
def get_task(task_id: str = FPath(..., description="Task ID: task1, task2, or task3")):
    if task_id not in TASKS:
...
def reset_env(task_id: str = FPath(..., description="Task ID to reset")):
...
def step_env(
    action: Action,
    task_id: str = FPath(..., description="Task ID"),
):
    """
...
def get_state(task_id: str = FPath(..., description="Task ID")):
